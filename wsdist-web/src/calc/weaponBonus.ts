/**
 * Weapon skill damage bonuses from specific weapons (REMA, Ambuscade, Aeonic, etc.)
 * Port of weapon_bonus.py
 */

const WEAPON_SKILL_BONUSES: Record<string, Record<string, number>> = {
  // Ambuscade weapons
...
export function getWeaponBonus(mainWpnName: string, rngWpnName: string, wsName: string): number {
  let wsBonus = 0
  if (mainWpnName in WEAPON_SKILL_BONUSES) {
    wsBonus += WEAPON_SKILL_BONUSES[mainWpnName][wsName] ?? 0
  }
  if (rngWpnName in WEAPON_SKILL_BONUSES) {
    wsBonus += WEAPON_SKILL_BONUSES[rngWpnName][wsName] ?? 0
  }
  return wsBonus
}
