...
const wsNameOptions = computed(() => {
  const list = WS_BY_JOB[characterStore.mainJob] ?? []
...
const mainJobModel = computed({
  ...
  set: (v: string) => {
    ...
    // Auto-select a valid WS for the new job
    const available = WS_BY_JOB[v] ?? []
    if (!available.includes(characterStore.wsName)) {
      characterStore.wsName = DEFAULT_WS_BY_JOB[v] ?? available[0] ?? ''
    }
  },
})
